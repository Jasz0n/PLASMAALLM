...
| **Ask ALLM (grounded chat)** | **done (2026-07-09).** The approachable surface — and a demonstration of the thesis, not a betrayal of it. `allm.ask.answer_question` answers **only** from the evidence graph, always with confidence + provenance + contested-ness, and says *"I don't have evidence about that yet — I won't guess"* when nothing matches (deterministic retrieval; a local-LLM phrasing layer is an opt-in later). `GET /ask?q=…` (open read), a self-contained `/chat` UI served like the dashboard (subpath-robust), `AllmClient.ask()`, `tests/test_ask.py`. Status per answer: established / emerging / contested / unfounded / unknown — an unfounded doc-only claim is flagged *"a document proposes, evidence disposes"*. **Intent-aware:** a *how-to* question is answered by the **reproducible procedure** behind the evidence (an evidence package's reproduction steps — grounded, replicated), and when no steps were submitted it refuses to invent them (`no_procedure`) rather than returning the definition. **Grounded RAG (`ALLM_ASK_MODEL`):** with a local model configured, it *understands* the question (distinguishes "how do I make X" from "what X methods exist") and composes the answer **only from the retrieved evidence** with a strict no-outside-knowledge prompt — the facts, confidence and provenance are still computed from the graph, not the model, and it falls back to extraction when the model is unset/unreachable. Verified live on qwen2.5:7b (`answer_with_model`, `tests/test_ask.py` with a fake model so CI needs no Ollama) |
...
| **Seed & demo scenario** | **done (2026-07-09).** `allm seed --db …` runs the whole public loop for real through the actual subsystems (not fabricated rows): a Keshe-flavoured corpus where two workshops disagree on a number → KDP surfaces the **conflict** on `The Nano Coating` → **proposal** → three independent **replications** → confidence shifts **0.41 → 0.79**, resolved *supported*, all on the live feed with KEL measurements — so a fresh deploy's dashboard is immediately alive. Refuses to clobber without `--force`. CI rehearses the loop end-to-end (`allm.seed`, `tests/test_seed.py`), so the exit-criterion path is proven before real contributors arrive |
