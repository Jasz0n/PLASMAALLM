...
| **Ask ALLM (grounded chat)** | **done (2026-07-09).** The approachable surface — and a demonstration of the thesis, not a betrayal of it. `allm.ask.answer_question` answers **only** from the evidence graph, always with confidence + provenance + contested-ness, and says *"I don't have evidence about that yet — I won't guess"* when nothing matches (deterministic retrieval; a local-LLM phrasing layer is an opt-in later). `GET /ask?q=…` (open read), a self-contained `/chat` UI served like the dashboard (subpath-robust), `AllmClient.ask()`, `tests/test_ask.py`. Status per answer: established / emerging / contested / unfounded / unknown — an unfounded doc-only claim is flagged *"a document proposes, evidence disposes"*. **Intent-aware:** a *how-to* question is answered by the **reproducible procedure** behind the evidence (an evidence package's reproduction steps — grounded, replicated), and when no steps were submitted it refuses to invent them (`no_procedure`) rather than returning the definition |
