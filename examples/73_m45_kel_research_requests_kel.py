"""M45 capstone — KEL research requests close the loop to Researcher.

    PYTHONPATH=src python3 examples/73_m45_kel_research_requests_kel.py
...
def main() -> None:
    setup_logging("INFO")
    _apply_defaults()
    parser = argparse.ArgumentParser(description="M45 KEL research requests capstone")
    ...
    print(f"  vision model: {vision_model} (ollama reachable: {ollama_reachable()})")
    print(format_corpus_audit(entries))
    if not corpus_is_complete(entries):
        raise SystemExit("Book corpus incomplete.")

    workdir = Path(tempfile.mkdtemp(prefix="allm-m45-"))
    ...
    print("\n  Check logs for: KEL submitted N research request(s) to Researcher")
